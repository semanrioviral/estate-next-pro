...
const getFeedProperties = unstable_cache(
    async (): Promise<Property[]> => {
        ...
        if (error) {
            console.error('[google-feed] Error fetching properties:', error.message);
            return [];
        }
...
function mapCondition(tipo: string | null | undefined): 'new' | 'used' {
    return tipo === 'proyecto' ? 'new' : 'used';
}

function formatGooglePrice(value: number | null | undefined): string {
    if (typeof value !== 'number' || Number.isNaN(value)) {
        return '0.00 COP';
    }

    return `${value.toFixed(2)} COP`;
}

function ensureJpgForGoogle(url: string): string {
    const heicMatch = url.match(/^(.*\/image\/upload\/)(.*)$/);
    if (!heicMatch) {
    ...
    const prefix = heicMatch[1];
    const rest = heicMatch[2];
    if (rest.endsWith('.heic') || rest.endsWith('.heif')) {
        return `${prefix}f_jpg/${rest.replace(/\.(heic|heif)$/i, '.jpg')}`;
    }
...
export async function GET(): Promise<Response> {
    ...
    const items = properties
        ...
        .map((property) => {
            ...
            const condition = mapCondition(property.tipo);
            ...
            const mainImage = ensureJpgForGoogle(toAbsoluteUrl(property.imagen_principal));
            const propertyLink = `${BASE_URL}/propiedades/${property.slug}`;

            const additionalImages = (property.galeria || [])
                .filter((imageUrl) => Boolean(imageUrl))
                .map((imageUrl) => ensureJpgForGoogle(toAbsoluteUrl(imageUrl)))
            ...
            return [
                ...
                additionalImageTags,
                tag('g:price', formatGooglePrice(property.precio)),
                tag('g:availability', availability),
                tag('g:brand', 'Tucasa Los Patios'),
    ...
    const xml = [
        ...
        '<channel>',
        '<title>Tucasa Los Patios - Google Catalog</title>',
        `<link>${BASE_URL}</link>`,
